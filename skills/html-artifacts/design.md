# Geist (Vercel) — Design System Reference

> Rendered in `design-reference.html`. Geist Sans / Geist Mono are not system fonts, so it
> declares them first and falls back to Inter / JetBrains Mono / system stacks per the
> substitution note below — no webfont is fetched, keeping artifacts offline-safe.
>
> **Marks are out of scope.** This document describes a *visual system* — palette, type
> scale, spacing, radii, component shapes — and nothing here licenses Vercel's trademarks.
> Never render the Vercel triangle or any other borrowed logo in an artifact. Where a spec
> below says "wordmark," set the project's own name as plain text in
> `{typography.label-sm}`; do not draw a mark and do not leave a logo placeholder.
...
> **Reconciliation.** As specified, this is not buildable: `{components.button-primary-sm}` has `0px 6px` padding and `{typography.button-md}` line-height, which cannot reach 44px. The skill resolves it by giving marketing pills an explicit 44px height and app-surface squares 32px with 10px horizontal padding — see `references/matching-your-style.md`. Follow that, not the sentence above.
...
- Background `{colors.canvas}`, bottom hairline `{colors.hairline}`, text `{colors.body}`, type `{typography.body-md}`, padding `{spacing.sm} {spacing.lg}`. Holds the wordmark (plain text — see the note at the top), ghost nav links, and the Sign Up / Log In buttons at right.
